...
INPUT_DIR  = r"video\video_input_vertical"   # Папка с исходниками (вертикальные)
OUTPUT_DIR = r"video\video_output_vertical"  # Папка для готовых видео (вертикальные)

# Целевое разрешение HD для вертикального видео (Reels / TikTok / Shorts)
# Скрипт растянет результат кропа до этого размера.
# Примеры: (1080, 1920) — Full HD вертикаль, (720, 1280) — HD vертикаль
TARGET_W = 1080
TARGET_H = 1920
...
def process_video(video_path: Path, index: int, total: int) -> None:
    ...
    # scale= → растягивает результат до целевого HD-разрешения
    vf_filter = (
        f"crop={crop_w}:{crop_h}:{crop_x}:{crop_y},"
        f"scale={TARGET_W}:{TARGET_H}:"
        f"flags=lanczos"   # lanczos — самый качественный алгоритм апскейла
    )
    ...
    cmd = [
        ...
        '-preset', 'veryslow',     # veryslow даёт максимальное качество сжатия
        '-crf', '16',              # 16 = практически студийный lossless (очень высокое качество)
        '-pix_fmt', 'yuv420p',     # Совместимость с iOS / Instagram / YouTube
        *build_audio_args(),
...
if total == 0:
    ...
    if errors == 0:
        ...
        tg(f"🎬 <b>Video Crop Vertical Complete</b>\n"
