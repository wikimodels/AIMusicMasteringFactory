...
orig_path   = ROOT / "Viola da Gamba v7 Clarity.wav"
master_path = ROOT / "sound" / "wav_output" / "Viola da Gamba v7 Clarity_middle.wav"
out_path    = ROOT / "analysis" / "Viola_da_Gamba_v7_Clarity_analysis.png"

print(f"Original exists : {orig_path.exists()}  -> {orig_path}")
print(f"Master exists   : {master_path.exists()} -> {master_path}")
...
print(f"Samples loaded: {n}  sr={sr}")
...
BG, GR = "#1a1a1a", "#2a2a2a"
ORIG, MASTER = "#7ec8e3", "#c084fc"

fig = plt.figure(figsize=(14, 10), facecolor="#0d0d0d")
fig.suptitle("Viola da Gamba v7 Clarity — EQ Middle Fix Analysis",
...
ax1.semilogx(fm[mask], dm[mask], color=MASTER, lw=1.5, label="+ Mid Fix", alpha=0.85)
...
ax2.plot(tm, rm, color=MASTER, lw=1.0, label="+ Mid Fix", alpha=0.8)
...
sa(ax3, "Spectrum Delta (Mid Fix - Original)")
